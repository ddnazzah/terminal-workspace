export interface FuzzyResult {
  score: number
  /** Code-point indices in the target that matched, in order. */
 ...
 *
 * Works in Unicode code points (not UTF-16 units) so `matchedIndices` line up
 * with code-point iteration in the renderer's highlighter.
 */
export function fuzzyMatch(query: string, target: string): FuzzyResult | null {
  ...
  for (let ti = 0; ti < t.length && qi < q.length; ti++) {
    ...
    if (isBoundary(targetChars, ti)) charScore += BOUNDARY_BONUS
